...
test_reactions = ['Exchange_AMMONIA', 'Exchange_AMMONIA_REV',
                  'Exchange_UREA', 'Exchange_UREA_REV',
                  'Exchange_OXYGEN-MOLECULE', 'Exchange_OXYGEN-MOLECULE_REV',
                  'Exchange_Glucopyranose'
                  ]
...
def fva_special(model_go_pools, results02, test_conds, dftot, dffrac, output_file):
    result_df = pd.DataFrame(
        columns=['RXN_id', 'O2_bound', 'kcat_index', 'condition_id', 'growth', 'minimum', 'maximum'])
        ...
        for kcat in results02.population:
            ...
            with model:
                ...
                for condition_id in test_conds:
                    ...
                    model.reactions.get_by_id('Exchange_OXYGEN-MOLECULE').bounds = (o2_bound*0.99,o2_bound*1.01)
                    ...
                    try:
                        if r1 != -1:
                            result_df_i_pro = flux_variability_analysis(model, test_reactions,
                            ...
                            result_df_i_pro['RXN_id'] = result_df_i_pro.index
                            result_df_i_pro['O2_bound'] = o2_bound
                            result_df_i_pro['kcat_index'] = kcat_number
                            result_df_i_pro['condition_id'] = condition_id
                            result_df_i_pro['growth'] = r1
                            result_df = result_df.append(result_df_i_pro)
...
p1 = multiprocessing.Process(target=fva_special, args=(
    model_go_pools, results02, test_conds, dftot, dffrac_pro, '../Results/FVA_result_df_pro_1228.tsv'))

p2 = multiprocessing.Process(target=fva_special, args=(
    model_go_pools, results02, test_conds, dftot, dffrac_mRNA, '../Results/FVA_result_df_mRNA_1228.tsv'))
p1.start()
p2.start()
